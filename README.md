...
![Sentinel 使用量・コスト レポートのサンプル](sample-img/report-sample.png)
...
Logic App（スケジュール実行）
 ...
 ├─ KQL_candidates        注目テーブル（急増/受信停止/tier 候補）のみを KQL で事前抽出（最大 15 件、SCU 削減）
 ├─ Select_candidate_lines 候補をパイプ区切り 1 行に圧縮して入力トークンを削減
 ├─ SecurityCopilot_insights   TOTALS（全体集計）＋CANDIDATES（注目テーブル）を Security Copilot Agent へ送り分析
 ├─ Normalize_agent_text  応答からコードフェンス・CR を除去し前後をトリム
 ├─ Compose_safe_insights Agent 出力をそのまま掲載用に整形（改行→<br>）。空応答時はフォールバック文を充当（数値は影響なし）
 ├─ Select_detail_rows    明細を決定論で生成（数値は KQL のままでLLMによる修正は行わせない）
 ├─ Compose_html          テンプレに数値（決定論）+ AI 分析（Agent 出力をそのまま）を差し込み
...
| `surgeThresholdPercent` | `100` | **オプション**。使用量急増とみなす前期比のしきい値（%）。この値以上のテーブルを「急増候補」として Agent 入力に含める。既定 100（+100% / 2 倍以上） |
| `deadDays` | `30` | **オプション**。受信停止候補とみなす最終受信からの経過日数。`lastSeen` がこの日数以上前のテーブルを Agent 入力に含める。既定 30（`Usage` は最大 90 日保持） |
...
KQL_candidates       注目テーブル（急増/受信停止/tier 候補）のみを事前抽出（最大 15 件）
Select_chart_rows    ② Top10 棒グラフ <div> を生成（幅 = 最大値比）
Select_detail_rows   ④ 明細 <tr> を生成
Compose_html         テンプレの {{TOKEN}} 19 個を replace() 連鎖で実値に差替（AI 分析は {{AGENT_INSIGHTS}} にそのまま掲載）
